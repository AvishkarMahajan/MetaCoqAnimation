...
Fixpoint type_to_eq_fn (t : term) : term :=
  ...
  | tApp <%prod%> [tp1 ; tp2] => tApp <%eqb_prod%> [tp1; tp2; (type_to_eq_fn tp1); (type_to_eq_fn tp2)]
  | tApp
...
Fixpoint ordered_vars (t : term) : list string :=
  ...
  | tApp <%eq%> [typeT; tVar str1; tApp fn lst] => str1 :: (app (ordered_vars fn) (concat (map ordered_vars lst)))
  | tApp <%eq%> [typeT; tApp fn lst; tVar str1] => app (app (ordered_vars fn) (concat (map ordered_vars lst))) [str1]
...
Definition animate_one_conj (conj : term) (knownVar : list string) (partialProg : term -> term) : option (list string * (term -> term)) :=
  match conj with
  | tApp <%eq%> [typeT; t1; t2] => if andb (is_subset_strings (ordered_vars t2 ) knownVar) (negb (is_subset_strings (ordered_vars t1 ) knownVar)) then
    ...
    else (if andb (is_subset_strings (ordered_vars t1 ) knownVar) (negb (is_subset_strings (ordered_vars t2 ) knownVar)) then
...
Definition compile_let_clause {A : Type} (induct : A) (kn : kername) (fooTerm : named_term)  (inputTm : term) (inputTp : term)  (outputTm : term) (outputTp : term) (inputVars : list string) (fuel : nat) : TemplateMonad term :=
...
Definition compile_clause {A : Type} (induct : A) (kn : kername) (conjunct' : (term * (string * term))) (modes : list (string * ((list nat) * (list nat)))) (predTypeInf : list (string * (list term))) (allVarTpInf : list (string * term)) (fuel : nat) : TemplateMonad term :=
...
let conjunct := fst conjunct' in
  ...
  | Some (indNm, lstArgs) =>
                     ...
                     inputVarProdTm <- mk_lhs_term inputVarsTmTp ;;
                      ...
                      tIn' <- join_pattern_fueled induct (inputVarProdTm) (inputVarProdTp) predInProdTm predInProdTp (String.append (snd kn) "IN") fuel ;;
                      (* Compose: user input -> pred input -> pred output (via recursive call) *)
                      let tIn := (tApp <%compose_outcome%> [(inputVarProdTp); predInProdTp ; (predOutProdTp) ; tIn' ; (tVar (String.append indNm top_fn_suffix))]) in
                      (* Build the output-side pattern match: maps predicate output to user output *)
                      tOut <- join_pattern_fueled induct predOutProdTm predOutProdTp (outputTm) (outputTp) (String.append (snd kn) "OUT") fuel ;;
                      (* Final composition: user input -> pred output -> user output *)
                      let u := (tApp <%compose_outcome%> [(inputVarProdTp); predOutProdTp ; (outputTp) ; tIn ; tOut]) in
                      u'' <- tmEval all u ;;
...
Definition animate_no_input {A : Type} (induct : A) (kn : kername) (conjunct' : (term * (string * term))) (modes : list (string * ((list nat) * (list nat)))) (predTypeInf : list (string * (list term))) (allVarTpInf : list (string * term)) (fuel : nat) : TemplateMonad term :=
...
let conjunct := fst conjunct' in
  ...
  | Some (indNm, lstArgs) =>
                     ...
                     inputVarProdTm <- mk_lhs_term_monadic inputVarsTmTp ;;
                      ...
                      tIn' <- join_pattern_fueled induct (inputVarProdTm) (inputVarProdTp) predInProdTm predInProdTp (String.append (snd kn) "IN") fuel ;;
                      let tIn := (tApp <%compose_outcome%> [(inputVarProdTp); predInProdTp ; (predOutProdTp) ; tIn' ; (tLam "fuel" <%nat%> (tLam "input" <%animation_result bool%> (tApp (tVar (String.append indNm top_fn_suffix))[tVar "fuel"])))]) in
                      tOut <- join_pattern_fueled induct predOutProdTm predOutProdTp (outputTm) (outputTp) (String.append (snd kn) "OUT") fuel ;;
                      let u := (tApp <%compose_outcome%> [(inputVarProdTp); predOutProdTp ; (outputTp) ; tIn ; tOut]) in
                      u'' <- tmEval all u ;;
                      u' <- tmEval all (typeConstrPatMatch.unwrap_option (DB.de_bruijn_option u)) ;;

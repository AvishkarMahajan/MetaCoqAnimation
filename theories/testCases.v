    ...
    tApp <%and%>
      [tApp <%eq%> [<%nat%>; tVar "d"; tVar "c"];
       tApp <%and%>
         [tApp <%eq%>
            [<%nat%>; tVar "c";
             tApp (tConst (MPfile ["testCases"; "Animation"], "g3") []) [tVar "a"; tVar "e"]];
          tApp <%eq%>
            [<%nat%>; tApp (tConst (MPfile ["testCases"; "Animation"], "g1") []) [tVar "d"];
             tApp (tConst (MPfile ["testCases"; "Animation"], "g2") []) [tVar "a"]]]]]).
...
(* 6 *) (* Failing test case : conjunct b = e cannot be animated since known var 'b' is on LHS not RHS *)

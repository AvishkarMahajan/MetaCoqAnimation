...
MetaRocq Run (animate_coinductive_transparent <? prodRel ?>
  [("prodRel", ([0], [1]))] 200).

Compute prodRelTransparentAnimatedTopFn 10 (Success (list nat * list nat) ([2;3],[8])).  
End prodRel.
...
MetaRocq Run (animate_coinductive_transparent <? mixedRel ?>
  [("mixedRel", ([0], [1]))] 200).
  
Compute mixedRelTransparentAnimatedTopFn 10 (Success (list nat) ([2;3])).  
End mixedRel. 
...
MetaRocq Run (animate_coinductive_transparent <?bigStepTr?>
...
Compute bigStepTrTransparentAnimatedTopFn 25 (Success tm omega).

Compute bigStepTrTransparentAnimatedTopFn 25 (Success tm (tif (tapp (tabs "x" TBool (tvar "x")) ttrue) tfalse ttrue)).              
End STLCStepTr.
...
MetaRocq Run (animate_coinductive_transparent <? zipSt ?>
  [("zipSt", ([0;1], [2]))] 100).
...
MetaRocq Run (animate_coinductive_transparent <? eqSt ?>
  [("eqSt", ([0;1], [2]))] 100).

Compute (eqStTransparentAnimatedTopFn 24 (Success (stream * stream) (from 8, from 8))).
Compute (eqStTransparentAnimatedTopFn 12 (Success (stream * stream) (from 8, from 9))).
End eqSt.
(* ------------------------------------------------------------------ *)
Module isEven.
...
MetaRocq Run (animate_coinductive_transparent <? Integrate ?>
  [("Integrate", ([0], [1])); ("addStm", ([0;1], [2])); ("plus", ([0;1], [2]))] 100).

(** Integrate [4, 5, 6, …] gives [4, 9, 15, …] (prefix sums). *)
Compute (IntegrateTransparentAnimatedTopFn 25 (Success stream (from 4))).
...
MetaRocq Run (animate_coinductive_transparent <? evalCmd ?>
  [("evalCmd", ([0;1], [2]))] 500).

Definition prog  := While (Var 4) (Assign 8 (Const 8)).
Definition initFn := pure (fun m : nat => m + 1).
Compute (evalCmdTransparentAnimatedTopFn 35 (Success (co_vars * cmd) (initFn, prog))).
...
MetaRocq Run (animate_coinductive_transparent <? stack_step ?>

(** * TestCoinductiveSigma2: Tests for animate_coinductive_transparent_sigma2.
...
MetaRocq Run (animate_coinductive_transparent_sigma2 <? filterEven ?>
  [("filterEven", ([0], [1]))] 100).
...
MetaRocq Run (animate_coinductive_transparent_sigma2 <?bigStepTr?>
...
Eval cbv -[HoleyResult.hlist_head bigStepTrAn1Symb_unwrap] in bigStepTrTransparentSigma2AnimatedTopFn 25 (Success tm omega).
...
MetaRocq Run (animate_coinductive_transparent_sigma2 <? Integrate ?>
  [("Integrate", ([0], [1])); ("addStm", ([0;1], [2]))] 100).

(** Integrate [4, 5, 6, …] gives [4, 9, 15, …] (prefix sums). *)
Eval cbv -[HoleyResult.hlist_head HoleyResult.hlist_tail IntegrateAn1Symb_unwrap addStmAn2Symb_unwrap] in (IntegrateTransparentSigma2AnimatedTopFn 5 (Success stream (from 4))).
...
CoInductive evalCmd : state -> cmd -> state -> Prop :=
| EvalAssign     : forall  vs v e,
     evalCmd (pure vs) (Assign v e) (pure (set vs v (evalExp vs e)))
| EvalSeq        : forall vs1 vs2 vs3 c1 c2,
    evalCmd vs1 c1 vs2 /\ evalCmd vs2 c2 vs3
    -> evalCmd vs1 (Seq c1 c2) vs3
| EvalWhileFalse : forall vs' vs e c, vs' = pure vs /\
     evalExp vs e = 0
    -> evalCmd vs' (While e c) vs'
| EvalWhileTrue  : forall vs1' vs2' vs3'  e c k,
     evalExp vs1' e  = S k
    /\ evalCmd (pure vs1') c vs2' /\ evalCmd vs2' (While e c) vs3'
    -> evalCmd (pure vs1') (While e c) vs3'.
...
Definition initFn : state := pure (fun m : nat => m + 1).
Print evalCmdAnimatedTopFnProp.
Eval cbv -[HoleyResult.hlist_head] in
  (evalCmdTransparentSigma2AnimatedTopFn 35 (Success ((state) * cmd) (initFn, prog))).
...
Definition initFn'' : state := pure (fun m : nat => m).

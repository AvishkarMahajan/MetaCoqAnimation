...
(* ------------------------------------------------------------------ *)
(** ** Guard-free CBV big-step evaluation                             *)
(* ------------------------------------------------------------------ *)
...
(** CBV application: evaluate operator to a lambda, evaluate argument,
    then evaluate the substituted body. No [isValueFn v2 = true] guard:
    [eval t2 v2] already guarantees [v2] is a value. *)
| E_App : forall t1 t2 x T t3 v2 v,
...
Theorem correspondence_soundness : forall (n : nat) (inputTm outputTm : tm),
  (evalTransparentSigma2AnimatedTopFn n (Success tm inputTm)) (fun t' : tm => t')  = Success (tm) (outputTm) ->
  inputTm = outputTm \/ stepTC inputTm outputTm.
Proof. Admitted.

Theorem correspondence_soundness_mono : forall (n m : nat) (inputTm outputTm outputTm2 : tm),
  m > n -> 
  (evalTransparentSigma2AnimatedTopFn n (Success tm inputTm)) (fun t' : tm => t')  = Success (tm) (outputTm) ->
  (evalTransparentSigma2AnimatedTopFn m (Success tm inputTm)) (fun t' : tm => t')  = Success (tm) (outputTm2) ->
  outputTm = outputTm2 \/ stepTC outputTm outputTm2.
Proof. Admitted.
...
(*                                                                     *)
(*  Reuses the [tm]/[ty]/[subst] syntax from [PCFBigStep] unchanged.  *)
(*                                                                     *)
(*  1. [eval] : the same CBV big-step relation as [PCFBigStep.eval],  *)
(*     but with the explicit [isValueFn _ = true] side-conditions     *)
(*     dropped. Those guards are redundant: the shape of the rules    *)
(*     already forces the second argument of [eval] to always be a   *)
(*     syntactic value (tabs / tzero / tsucc-spine), so "eval relates *)
(*     a term to a value" holds implicitly without being checked.     *)
(*                                                                     *)
(*  2. [step] : full beta reduction -- a non-deterministic congruence *)
(*     closure that permits reduction in every subterm position,      *)
(*     including under binders ([tabs], [tfix] bodies) and inside     *)
(*     both branches of [tifz], and where the beta/delta redexes      *)
(*     ([FB_AppAbs], [FB_PredSucc], ...) may fire without requiring   *)
(*     their argument/discriminant to already be a value (unlike the  *)
(*     CBV [PCFBigStep.step]).                                         *)
(* ================================================================== *)

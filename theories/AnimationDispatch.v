...
Definition animate_let_binding {A : Type} (ind : A) (kn : kername) (conjunct' : (term * (string * term))) (inputTm : term) (inputTp : term)
...
match conjunct with
 | tApp <%eq%> [typeVar; t1; t2] => match t1 with
                                    | tVar str =>  match inputVars with
                                                    ...
                                                    | h :: rest => compile_let_clause ind kn conjunct inputTm inputTp outputTm outputTp inputVars fuel
                                                   end
                                    | tApp (tConstruct ind_type k lst) lstArgs => compile_eq_binders_with_vars ind kn conjunct inputTm inputTp outputTm outputTp inputVars fuel
...
Definition build_let_chain_step (outputVarNm : string) (outputVarTp : term) (inputVarsLst : list (prod term term)) (animationFn : term) (partialLetfn : term -> term) : (term -> term) :=
  ...
  | _ =>  (fun t => partialLetfn ((tLetIn {| binder_name := nNamed outputVarNm; binder_relevance := Relevant |}
                                 (tApp animationFn [(tVar "fuel"); (tApp (mk_join_tm (map snd inputVarsLst)) (map fst inputVarsLst))]) (tApp <%animation_result%> [outputVarTp])) t))
  end.
...
Definition animate_one_let {A : Type} (ind : A) (kn : kername) (conjunct' : (term * (string * term))) (inputVarsLst : list (prod string term)) (partialLetfn : term -> term)
...
animationFn <-  animate_let_binding (ind) (kn) (conjunct') (inputTm) (inputTp) (map fst inputVarsLst) (modes) (predTypeInf) (allVarTpInf) fuel ;;
tmReturn (build_let_chain_step (outputVarNm) (outputVarTp) (inputVarsLstTm) (animationFn) (partialLetfn)).
...
Definition animate_one_guard {A : Type} (ind : A) (kn : kername) (conjunct' : (term * (string * term))) (inputVarsLst : list (prod string term)) (partialGuard : term)
...
animationFn <- animate_let_binding (ind) (kn) (conjunct') (inputTm) (inputTp)
                                 ...
                                 tmReturn (tApp <%and_outcome_bool%> [partialGuard ; tApp (mk_eq_outcome_tm outputVarTp (type_to_eq_fn outputVarTp)) [tVar outputVarNm ;
                                 (
                                 (tApp animationFn [(tVar "fuel"); (tApp (mk_join_tm (map snd inputVarsLst)) (map fst inputVarsLstTm))])) ]]).
...
Definition conj_vars_by_role (eqProj : term -> term -> list string) (modeProj : (list nat * list nat) -> list term -> list term) (conjunct' : (term * (string * term))) (allVarTpInf : list (prod string term)) (modes : list (string * ((list nat) * (list nat)))) (predTypeInf : list (string * (list term))) : list (prod string term) :=
let conjunct := fst conjunct' in
  match conjunct with
...
Definition conj_input_vars := conj_vars_by_role (fun _t1 t2 => ordered_vars t2) (fun mode lstArgs => select_in_by_mode mode lstArgs).

(** Animate one let-clause by first computing its input variable list from
    context, then delegating to [animate_one_let]. *)
Definition animate_let_with_ctx {A : Type} (ind : A) (kn : kername) (conjunct' : (term * (string * term))) (partialLetfn : term -> term) (modes : list (string * ((list nat) * (list nat)))) (predTypeInf : list (string * (list term))) (allVarTpInf : list (string * term)) (fuel : nat) : TemplateMonad (term -> term) :=
...
Definition animate_guard_with_ctx {A : Type} (ind : A) (kn : kername) (conjunct' : (term * (string * term))) (partialGuard : term) (modes : list (string * ((list nat) * (list nat)))) (predTypeInf : list (string * (list term))) (allVarTpInf : list (string * term)) (fuel : nat) : TemplateMonad (term) :=

let  inputVarsLst := conj_input_vars conjunct' allVarTpInf (modes) (predTypeInf) in
animate_one_guard ind kn conjunct' inputVarsLst partialGuard (modes) (predTypeInf) (allVarTpInf) fuel.
...
Fixpoint animate_let_list {A : Type} (ind : A) (kn : kername) (conjs : list (term * (string * term))) (partialLetfn : term -> term) (modes : list (string * ((list nat) * (list nat)))) (predTypeInf : list (string * (list term))) (allVarTpInf : list (string * term)) (fuel : nat) : TemplateMonad (term -> term) :=
  match conjs with
  | [] => tmReturn partialLetfn
  | h :: t => lFn' <- animate_let_with_ctx ind kn h partialLetfn (modes) (predTypeInf) (allVarTpInf) fuel ;; animate_let_list ind kn t lFn' (modes) (predTypeInf) (allVarTpInf) fuel
  end.

(** Animate a list of predicate guard conjuncts, threading the accumulated
    boolean guard [partialGuard] through each step. *)
Fixpoint animate_guard_list {A : Type} (ind : A) (kn : kername) (conjs : list (term * (string * term))) (partialGuard : term) (modes : list (string * ((list nat) * (list nat)))) (predTypeInf : list (string * (list term))) (allVarTpInf : list (string * term)) (fuel : nat) : TemplateMonad (term) :=
  match conjs with
  | [] => tmReturn partialGuard
  | h :: t => pg <- animate_guard_with_ctx ind kn h partialGuard (modes) (predTypeInf) (allVarTpInf) fuel ;; animate_guard_list ind kn t pg (modes) (predTypeInf) (allVarTpInf) fuel
  end.
...
Definition compile_guard_clause {A : Type} (induct : A) (kn : kername) (gConjsEq : list term)  (inputTm : term) (inputTp : term)  (outputTm : term) (outputTp : term) (fuel : nat) : TemplateMonad term :=
...
Definition animate_guard_eq {A : Type} (induct : A) (kn : kername) (gConjsEq : list term) (allVarTpInf : list (prod string term))  (outVars : list (prod string term)) (fuel : nat) : TemplateMonad term :=
compile_guard_clause induct kn gConjsEq  (tele_to_prod_tm allVarTpInf) (tele_to_prod_tp allVarTpInf) (tele_to_prod_tm outVars) (tele_to_prod_tp outVars) fuel.
...
Definition branch_on_bool (retType succTrueRetTm succFalseRetTm noMatchRetTm fuelErrorRetTm : term) : term :=
...
Definition animate_guard_bool_branch {A : Type} (ind : A) (kn : kername) (predGuardConjs : list (term × string × term))  (modes : list (string * ((list nat) * (list nat)))) (predTypeInf : list (string * (list term))) (allVarTpInf : list (string * term)) (outVars : list (prod string term)) (guardConEqAn : term) (fuel : nat) : TemplateMonad (term) :=
predGuardCon <- animate_guard_list (ind) (kn) (predGuardConjs) <%Success bool true%> (modes) (predTypeInf) (allVarTpInf) (fuel) ;;
let brOutBool := branch_on_bool (tApp <%animation_result%> [tele_to_prod_tp outVars]) (guardConEqAn) (tApp <%NoMatch%> [tele_to_prod_tp outVars]) (tApp <%NoMatch%> [tele_to_prod_tp outVars]) (tApp <%FuelError%> [tele_to_prod_tp outVars]) in
...
Fixpoint split_inputs (inVars : list (string * term)) (inTerm : term) (fnBody : term) : term :=
  ...
  | h' :: rest' =>  (tLetIn {| binder_name := nNamed (fst h'); binder_relevance := Relevant |}
                                 (tApp <% split_outcome_fst %> [(snd h'); (tele_to_prod_tp rest'); inTerm])  (tApp <%animation_result%> [(snd h')])) (split_inputs rest' (tApp <% split_outcome_snd %> [(snd h'); (tele_to_prod_tp rest'); inTerm]) fnBody)
  end.
...
Definition animate_lets_and_guards {A : Type} (ind : A) (kn : kername) (lConjs'' : list (term × (string × term))) (gConjsEq'' : list term) (gConjsPred'' : list (term × (string × term))) (inVars : list (prod string term)) (outVars : list (prod string term)) (modes : list (string * ((list nat) * (list nat)))) (predTypeInf : list (string * (list term))) (allVarTpInf : list (string * term)) (lhsPreds : list (string * term)) (fuel : nat) : TemplateMonad term :=
...
letBind <- animate_let_list  (ind) kn  lConjs  (fun t : term => t) (modes) (predTypeInf) (allVarTpInf) (fuel) ;;
...
combineGuard <- animate_guard_bool_branch (ind) (kn) (gConjsPred) (modes) (predTypeInf) (allVarTpInf) (outVars) (guardConEqAn) (fuel);;
  (* Wrap in lambdas for recursive function args, fuel, and input *)
  match inVars with
  | h :: rest => tmReturn (mk_lam_chain (app (mk_animated_names lhsPreds) [("fuel", <%nat%>)]) (tLam "input" (tApp <%animation_result%> [tele_to_prod_tp inVars])(split_inputs' inVars (letBind combineGuard))))
  | [] => tmReturn (mk_lam_chain (app (mk_animated_names lhsPreds) [("fuel", <%nat%>)]) (tLam "input" (tApp <%animation_result%> [<%bool%>]) (split_inputs' inVars (letBind combineGuard))))
...
Fixpoint classify_premises (modes : list (string * ((list nat) * (list nat)))) (currentConjs : list term) (remConjs : list term) (sortedConjs : list term) (guardConjs : list term) (kv : (list string)) (fuel : nat) : TemplateMonad (prod (list term) (list term)) :=
  match fuel with
  | 0 => if andb (is_nil remConjs) (is_nil currentConjs) then tmReturn (sortedConjs, guardConjs) else tmFail "insufficient fuel to sort conjs"
  | S n => if (andb (is_nil remConjs) (is_nil currentConjs)) then tmReturn (sortedConjs, guardConjs) else
           match currentConjs with
            | [] => classify_premises modes remConjs [] sortedConjs guardConjs kv n
            | conj' :: t => match conj' with
                        | tApp <%eq%> [typeVar; t1; t2] => if andb (is_subset_strings (ordered_vars t1) kv) (is_subset_strings (ordered_vars t2) kv) then
                                                            classify_premises modes t remConjs sortedConjs (conj' :: guardConjs) kv n else
                                                            (if (andb (is_subset_strings (ordered_vars t1) kv) (has_right_shape t2)) then

                                                            classify_premises modes t remConjs (tApp <%eq%> [typeVar; t2; t1] :: sortedConjs) (guardConjs) (app (ordered_vars t2) kv) n else
                                                            (if  (andb (is_subset_strings (ordered_vars t2) kv) (has_right_shape t1)) then classify_premises modes t remConjs (tApp <%eq%> [typeVar; t1; t2] :: sortedConjs) (guardConjs) (app (ordered_vars t1) kv) n else
                                                            (classify_premises modes t (conj' :: remConjs) (sortedConjs) (guardConjs) (kv) n)))

                        | _ => match get_ind_name conj' with
                               | Some (indNm, lstArgs) =>
                                   if is_subset_strings (concat (List.map ordered_vars lstArgs)) kv then classify_premises modes t remConjs sortedConjs (conj' :: guardConjs) kv n
                                   else if is_subset_strings (ordered_vars_of_list (select_in_args indNm modes lstArgs)) kv then classify_premises modes t remConjs (conj' :: sortedConjs) guardConjs (app (ordered_vars_of_list (select_out_args indNm modes lstArgs)) kv) n
                                   else classify_premises modes t (conj' :: remConjs) sortedConjs guardConjs kv n
                               | None => tmFail "incorrect conj shape"
                               end
                        end
           end
...
Definition get_classified (proj : list term * list term -> list term) (doReverse : bool) (modes : list (string * ((list nat) * (list nat)))) (currentConjs : list term) (remConjs : list term) (sortedConjs : list term) (guardConjs : list term) (kv : (list string)) (fuel : nat) : TemplateMonad (list term) :=
sConjs <- classify_premises modes (currentConjs) (remConjs) (sortedConjs) (guardConjs) (kv) (fuel) ;;
...
Definition conj_output_vars (conj : term) (allVarTpInf : list (prod string term)) (modes : list (string * ((list nat) * (list nat)))) (predTypeInf : list (string * (list term))) : list (prod string term) :=
conj_vars_by_role (fun t1 _t2 => ordered_vars t1) (fun mode lstArgs => select_out_by_mode mode lstArgs) (conj, (""%bs, <%bool%>)) allVarTpInf modes predTypeInf.

(** Pair each conjunct in [lconjs] with the output variables it introduces. *)
Fixpoint attach_output_vars (lconjs : list term) (allVarTpInf : list (prod string term)) (modes : list (string * ((list nat) * (list nat)))) (predTypeInf : list (string * (list term))) : (list (prod term (list (prod string term)))) :=
  match lconjs with
  | [] => []
  | h :: t => (h, conj_output_vars h allVarTpInf modes predTypeInf) :: attach_output_vars t allVarTpInf modes predTypeInf
  end.

(** Tag a single conjunct term [lconjt] with each output variable in [lconjV],
    producing one [(conjunct, output_var)] pair per variable. *)
Fixpoint attach_var_to_conj (lconjt : term) (lconjV : (((list (string * term))))) : list (term * (string * term)) :=
  match (lconjV) with
  | [] => []
...
Fixpoint attach_vars_to_conjs (lconjs : (list (prod term (list (prod string term))))) : list (term * (string * term)) :=
  match lconjs with
  | [] => []
...
Definition attach_sorted_outputs (lconjs : list term) (allVarTpInf : list (prod string term)) (modes : list (string * ((list nat) * (list nat)))) (predTypeInf : list (string * (list term))) : list (term * (string * term)) :=
...
Fixpoint remove_dup_defs (lconjs : list (term * (string * term))) (kv : list string) : list (term * (string * term))  :=
  match lconjs with
  | [] => []
  | h :: rest => if (in_strings (fst (snd h)) kv) then remove_dup_defs rest kv  else h :: (remove_dup_defs rest ((fst (snd h)) :: kv))
  end.
...
Fixpoint keep_dup_defs (lconjs : list (term * (string * term))) (kv : list string) : list (term * (string * term))  :=
  match lconjs with
  | [] => []
  | h :: rest => if (in_strings (fst (snd h)) kv) then h :: (keep_dup_defs rest kv)  else (keep_dup_defs rest ((fst (snd h)) :: kv))
  end.

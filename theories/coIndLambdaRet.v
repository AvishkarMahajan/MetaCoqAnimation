...
Definition make_outputPush_term
  ...
  let n_fn_args     := List.length fn_arg_types in
  (* Depth variable inside the success branch body.
     Binder stack above depth_var: 1 (n lambda) + 1 (res lambda) + N (fn_arg lambdas)
       + 1 (Success branch binder) + 2*(n_in-1) (pair-match binders) = 2*n_in + N.
     For n_in=1, N=0: depth_var = tRel 2 = tRel(2*1). *)
  let depth_var     := tRel (2 * n_in + n_fn_args) in
  (* fn_arg references inside the Success body.
     fn_0 is outermost (tRel (2*n_in + n_fn_args - 2)), fn_{N-1} is innermost (tRel (2*n_in - 1)).
     Formula: fn_arg_refs[k] = tRel (2*n_in + n_fn_args - 2 - k) for k in 0..N-1. *)
  let fn_arg_refs   := List.map (fun k => tRel (2 * n_in + n_fn_args - 2 - k)) (seq 0 n_fn_args) in
  let no_match_body := tApp (tConstruct anim_res_ind 2 []) [orig_type] in
  let pushed_vals :=
    mapi (fun i pfb =>
      match pfb with
      | Some (fn, true)  => tApp fn (fn_arg_refs ++ [input_var i n_in])
      | Some (fn, false) => tApp fn (fn_arg_refs ++ [depth_var; input_var i n_in])
      | None             => input_var i n_in
      end)
    push_fns in
  ...
  (* The case scrutinizes res. After adding N fn_arg lambdas between res and the case,
     res is at depth N from inside the case, so the scrutinee is tRel n_fn_args. *)
  let case_expr :=
    tCase
      {| ci_ind := anim_res_ind; ci_npar := 1; ci_relevance := Relevant |}
  ...
  let fn_body :=
    tLambda anon_b anim_in_type
      (List.fold_right (fun fn_t acc => tLambda anon_b fn_t acc) case_expr fn_arg_types) in
...
Definition find_hole_idx_in_kns (kn : kername) (ht_kns : list kername) : nat :=
  match find_first_index_opt (eq_kername kn) ht_kns 0 with
  | Some j => j
  | None   => 0
  end.
...
Definition make_transparent_sigma_push_body_def
  ...
  let kn_hole_refs_in_s := fun (n_args : nat) (kn : kername) =>
    ...
    List.map (fun h_t =>
      match h_t with
      | tInd ind _ => s_hole_ref n_args (find_hole_idx_in_kns (inductive_mind ind) ht_kns)
    ...
    mapi (fun ctor_idx ctor =>
      ...
      let bbody :=
        if Nat.ltb ctor_idx n_old_ctors then
          ...
          let push_arg_term := fun snoc_i =>
            ...
            | Some None =>
              if is_purely_ind then
                ...
                let coind_idx := find_hole_idx_in_kns coind_kn ht_kns in
                tApp (s_hole_ref n_args coind_idx) [tRel snoc_i]
            | Some (Some kn) =>
              ...
              | None =>
                if existsb (eq_kername kn) pi_set then
                  ...
                  let kn_coind_idx := find_hole_idx_in_kns kn_coind_kn ht_kns in
                  tApp (s_hole_ref n_args kn_coind_idx) [tRel snoc_i]
              end
            | None =>
          ...
          let w_idx  := find_hole_idx_in_kns w_kn ht_kns in
          let fn_ref := s_hole_ref n_args w_idx in
          match pushed with
          | [] => fn_ref
          | _  => tApp fn_ref pushed
          end
  ...
  if is_purely_ind then
    ...
    let o_branch_body := tApp o_coind_ref [tRel 0] in
    let o_branch := {| bcontext := []; bbody := o_branch_body |} in
    let s_branch := {| bcontext := [anon_b]; bbody := inner_case |} in
    let dbody :=
      List.fold_right (fun h_t acc => tLambda anon_b h_t acc)
...
(** The transparent-sigma push wrapper is simply the body constant itself.
    Its type is fn_type_0 -> ... -> fn_type_{N-1} -> new_type -> old_type. *)
Definition make_transparent_sigma_push_wrapper_term
    (old_kn : kername)
    (cur_mp : modpath)
    : term :=
  tConst (cur_mp, snd old_kn ++ "TransparentSigmaPushBody") [].
...
Polymorphic Fixpoint generate_transparent_sigma_push_fns
  ...
  | ((old_kn, new_ind), (old_mind, new_mind)) :: rest =>
    ...
    tmBind (match nth_error new_mind.(ind_bodies) (inductive_ind new_ind) with
            ...
            | Some new_oib =>
              let ht_kns_from_unique :=
                List.map (fun t => match t with
                                   | tInd ind _ => inductive_mind ind
                                   | _ => (cur_mp, "")
                                   end) unique_ht_terms in
              let d_body := make_transparent_sigma_push_body_def
                              ...
                              1 0 [] [] ht_kns_from_unique in
              body_ev <- tmEval all (tFix [d_body] 0) ;;
              _ <- tmMkDefinition (snd old_kn ++ "TransparentSigmaPushBody") body_ev ;;
              wrapper_ev <- tmEval all (make_transparent_sigma_push_wrapper_term old_kn cur_mp) ;;
  ...
  | entry :: rest =>
    ...
    | Some oib =>
      ...
      generate_transparent_sigma_outputPush_fns rest type_map app_kn_map pi_set arr_name_pairs
                               prod_kn anim_res_kn cur_mp fn_arg_types)
    end)
  end.
...
Definition make_arrow_transparent_push_body_def
  ...
  let kn_hole_refs_in := fun (n_args : nat) (kn : kername) =>
    ...
    List.map (fun h_t =>
      match h_t with
      | tInd hind _ => s_hole_ref n_args (find_hole_idx_in_kns (inductive_mind hind) ht_kns)
      ...
      let bbody :=
        if String.eqb ctor_nm lift_nm then tRel 0
          ...
          if string_is_suffix ctor_nm "LiftedCstr" then
            ...
            let w_kn := (cur_mp, ctor_nm ++ "Symb") in
            if existsb (eq_kername w_kn) ht_kns then
              let w_idx  := find_hole_idx_in_kns w_kn ht_kns in
              let fn_ref := s_hole_ref n_args w_idx in
              match pushed with [] => fn_ref | _ => tApp fn_ref pushed end
            else tVar ("unknown_arrow_ctor_" ++ ctor_nm)
      in
      {| bcontext := List.rev (List.map (fun d => d.(decl_name)) ctor.(cstr_args));
         bbody    := bbody |})
    new_oib.(ind_ctors) in
...
(** The arrow transparent-sigma push wrapper is simply the body constant itself. *)
Definition make_arrow_transp_push_wrapper
    (name   : string)
    (cur_mp : modpath)
    : term :=
  tConst (cur_mp, name ++ "TransparentSigmaPushBody") [].
...
Polymorphic Fixpoint generate_transp_push_in_order
    ...
    (pi_set         : list kername)
    (cur_mp         : modpath)
    (* ht_kns: sentinel knames for each fn arg (one per output position per relation).
       global_ht: fn types corresponding to ht_kns, used as unique_ht_terms for ALL push bodies.
       global_ph: every type kn maps to fake tInd sentinel terms built from ht_kns,
       so kn_hole_refs_in_s for any kn emits references to all global holes. *)
    (ht_kns    : list kername)
    (global_ht : list term)
        ...
        mapi (fun self_fix_idx '(kn_i, ind_i) =>
          ...
          | Some (old_kn, new_ind, new_oib, n_old_ctors, n_block, _) =>
            ...
            make_transparent_sigma_push_body_def
              ...
              n_mutual self_fix_idx mutual_siblings arr_block_map ht_kns
          end
        ) group in
      _ <- List.fold_left
        (fun acc_m i_pair =>
        ...
        | Some entry =>
          ...
          let d_body :=
            make_arrow_transparent_push_body_def
              ...
              global_ht global_ph cur_mp 1 0 [] arr_block_map ht_kns in
          body_ev <- tmEval all (tFix [d_body] 0) ;;
          _ <- tmMkDefinition (name ++ "TransparentSigmaPushBody") body_ev ;;
          wrapper_ev <- tmEval all (make_arrow_transp_push_wrapper name cur_mp) ;;
            ...
            | None =>
              ...
              | Some (arr_t, nm, fn_ind, _, new_oib, n_block, _) =>
                make_arrow_transparent_push_body_def
                  ...
                  global_ht global_ph cur_mp n_mutual self_fix_idx mutual_siblings arr_block_map ht_kns
              | None =>
                let anon_b := {| binder_name := nAnon; binder_relevance := Relevant |} in
                {| dname := anon_b; dtype := tVar "?"; dbody := tVar "?"; rarg := 0 |}
              end
          ...
          (fun acc_m i_pair =>
            ...
            | None =>
              ...
              | Some (arr_t, nm, fn_ind, _, new_oib, n_block, _) =>
                ...
                wrapper_ev <- tmEval all (make_arrow_transp_push_wrapper nm cur_mp) ;;
...
(** Remove the first occurrence of [needle] from [haystack], or return [haystack]
    unchanged if [needle] is not found. *)
Fixpoint strip_first_occurrence (needle haystack : string) (fuel : nat) : string :=
  match fuel with
  | 0 => haystack
  | S fuel' =>
    let nlen := String.length needle in
    let hlen := String.length haystack in
    if Nat.leb nlen hlen then
      if String.eqb needle (String.substring 0 nlen haystack) then
        String.substring nlen (hlen - nlen) haystack
      else
        String.substring 0 1 haystack ++
          strip_first_occurrence needle (String.substring 1 (hlen - 1) haystack) fuel'
    else haystack
  end.

(** Convert a hole sentinel kername to a human-readable binder name.
    E.g. ["evalCmdremoveFnPosAn2Symb"] → ["evalCmdAn2"]. *)
Definition fn_kn_to_binder_name (kn : kername) : string :=
  let s := snd kn in
  let slen := String.length s in
  let no_symb :=
    if Nat.leb 4 slen
    then if String.eqb "Symb" (String.substring (slen - 4) 4 s)
         then String.substring 0 (slen - 4) s
         else s
    else s in
  strip_first_occurrence "removeFnPos" no_symb (String.length no_symb).

(** Compute the sentinel kname list and corresponding fn-type list for all output
    positions of all relations in [modes].  Returns [(ht_kns, fn_types)] where
    [ht_kns.(k)] is the sentinel kname and [fn_types.(k)] is the fn type for the
    k-th output position (across all relations, in mode-list order).
    No inductives are declared; this purely computes the lambda parameter types. *)
Polymorphic Definition compute_anim_fn_types_for_modes
    (modes          : mode_map)
    (arr_name_pairs : list (term * string))
    (cur_mp         : modpath)
    : TemplateMonad (list kername * list term) :=
  let anon_b := {| binder_name := nAnon; binder_relevance := Relevant |} in
  pairs_raw <- monad_map (fun '(rel_name, (in_pos, out_pos)) =>
    refs <- tmLocate rel_name ;;
    match find (fun g => match g with IndRef _ => true | _ => false end) refs with
    | None => tmReturn []
    | Some (IndRef rel_ind) =>
      ...
      | Some oib =>
        let n_idx     := #|oib.(ind_indices)| in
        let arg_types := extract_arg_types_early rel_mind.(ind_npars) n_idx oib.(ind_type) in
        pairs <- monad_map (fun op =>
          match nth_error arg_types op with
          | None => tmReturn []
          | Some op_type =>
            let input_types :=
              flat_map (fun ip => match nth_error arg_types ip with
                                  | None => [] | Some t => [t] end) in_pos in
            let fn_type := List.fold_right (fun ty acc => tProd anon_b ty acc) op_type input_types in
            fn_type_ev <- tmEval all fn_type ;;
            let kn := (cur_mp, rel_name ++ "An" ++ string_of_nat op ++ "Symb") in
            tmReturn [(kn, fn_type_ev)]
          end) out_pos ;;
        tmReturn (List.concat pairs)
      end
    | Some _ => tmReturn []
    end)
  modes ;;
  let all_pairs := List.concat pairs_raw in
  tmReturn (List.map fst all_pairs, List.map snd all_pairs).

(** Generate [TransparentSigmaPush] functions for all types in the testing
    pathway.  Computes fn-type lambda parameters for animation constructors,
...
Polymorphic Definition generate_transparent_push_testing
  ...
  (* Compute fn-type lambda parameters: one per output position of each relation. *)
  '(ht_kns, global_ht) <- compute_anim_fn_types_for_modes modes arr_name_pairs cur_mp ;;
  (* Build global_ph: map each type kn to fake tInd sentinel terms built from ht_kns.
     kn_hole_refs_in_s extracts inductive_mind from these sentinels to look up hole indices. *)
  let sentinel_terms :=
    List.map (fun kn => tInd {| inductive_mind := kn; inductive_ind := 0 |} []) ht_kns in
  let all_kns :=
    List.app
      (List.map (fun '(old_kn, _, _, _, _, _) => old_kn) sigma2_data)
      (List.map (fun '(_, nm, _, _, _, _, _) => (cur_mp, nm)) arr_data) in
  let global_ph := List.map (fun kn => (kn, sentinel_terms)) all_kns in
  generate_transp_push_in_order sorted_groups sigma2_data arr_data
    type_mapping app_kn_mapping arr_name_pairs arr_block_map pi_set cur_mp
    ht_kns global_ht global_ph.
...
Polymorphic Definition animate_coinductive_with_fn_pos
  ...
  | _  =>
    ...
    | Some (IndRef prod_ind), Some (IndRef anim_ind) =>
      ...
      '(ht_kns, fn_arg_types) <- compute_anim_fn_types_for_modes modes_rfp arr_name_pairs cur_mp ;;
      let pi_set_all := List.map fst type_mapping in
      _ <- generate_transparent_sigma_outputPush_fns kn_mode_list type_mapping app_kn_mapping pi_set_all
                              arr_name_pairs prod_kn anim_res_kn cur_mp fn_arg_types ;;
      ...
      | Some (_, (in_pos, out_pos)), Some rfp_oib =>
        ...
        | Some (IndRef prod_ind2), Some (IndRef anim_ind2) =>
          ...
          (* Build composite: fun n res fn_0 ... fn_{N-1} =>
               TransparentSigmaOutputPush n (AnimFn n (inputLift n res)) fn_0 ... fn_{N-1}
             Type: nat -> anim_res in_type -> fn_type_0 -> ... -> fn_type_{N-1} -> anim_res orig_type
             fn_i binders are named from the corresponding sentinel kname, e.g. evalCmdAn2. *)
          let n_fn_args := List.length fn_arg_types in
          let n_ref     := tRel (n_fn_args + 1) in
          let res_ref   := tRel n_fn_args in
          let fn_refs   := List.map (fun k => tRel (n_fn_args - 1 - k)) (seq 0 n_fn_args) in
          let anim_result := tApp animFn [n_ref; tApp inputLift_fn [n_ref; res_ref]] in
          let fn_binders  :=
            List.map (fun kn =>
              {| binder_name := nNamed (fn_kn_to_binder_name kn);
                 binder_relevance := Relevant |}) ht_kns in
          let composite :=
            tLambda anon_b (tInd nat_ind [])
            (tLambda anon_b anim_in_type
            (List.fold_right (fun '(fn_t, b) acc => tLambda b fn_t acc)
              (tApp transparentSigmaPush_fn ([n_ref; anim_result] ++ fn_refs))
              (combine fn_arg_types fn_binders)))
          in

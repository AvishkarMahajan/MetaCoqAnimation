...
Inductive even : nat -> bool -> Prop := (* mode = ([0], [1] *)
 ...
 | evenSucc : forall (w w1 : nat) (b : bool), odd w true /\ w1 = (S w) /\ b = true -> even w1 b
with odd : nat -> bool -> Prop :=
 | oddSucc : forall (w w1 : nat) (b : bool), b = true /\ even w b /\ w1 = (S w) -> odd w1 b. 
...
MetaRocq Run (g <- getData' <? append ?> [("append", ([0;1], [2]))] ;; tmDefinition "dataApp" g).

Compute (dataApp).

MetaRocq Run (g <- getData' <? even ?> [("even",([0], [1])); ("odd", ([0],[1]))] ;; tmDefinition "dataEven" g).

...
match p with 
|{|ind_finite := inf; ind_npars := n; ind_params := ip; 
                                                          ind_bodies := ({| ind_name := inn; ind_indices := ii; ind_sort := is; ind_type := it; ind_kelim := ik; ind_ctors := ({|cstr_name := cname;  cstr_args := ({| decl_name := dname; decl_body := dbody; decl_type := (tApp te (a0 :: (a1 :: ((tApp j1 j2) :: atail))))|} :: bcarg);  cstr_indices := ltm;  cstr_type := ct;  cstr_arity := j|} :: bctors);
                                                                            ind_projs := ipr; ind_relevance := irl|} :: tb); ind_universes := iu; ind_variance := iv|}  => 
             Some j1
| _ => None
end.    
